...
function BarChart({ data, height = 160 }: { data: { fecha: string; total_dia: number; cantidad: number }[]; height?: number }) {
...
function HBarChart({ data, valueKey, labelKey, color = "rgba(45,106,79,.75)", formatValue }: { data: Record<string, number | string>[]; valueKey: string; labelKey: string; color?: string; formatValue?: (v: number) => string }) {
...
function StatCard({ label, value, sub, icon, delta }: { label: string; value: string; sub?: string; icon: IconName; delta?: number | null }) {
  return (
...
function Card({ title, children, style }: { title?: string; children: React.ReactNode; style?: CSSProperties }) {
  return (
...
function LegendPill({ label, value, color }: { label: string; value: string | number; color?: string }) {
  return (
...
function StatDescRow({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
...
const s: Record<string, CSSProperties> = {
  ...
  statLabel: { fontSize: "0.72rem", fontWeight: 600, color: "var(--muted)", textTransform: "uppercase" as const, letterSpacing: "0.05em" },
  statValue: { fontFamily: "var(--font-head)", fontSize: "1.55rem", fontWeight: 700, color: "var(--accent)", lineHeight: 1.1 },
  statSub: { fontSize: "0.72rem", color: "var(--muted)" },
  card: { background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "1.1rem 1.25rem" },
  cardTitle: { fontFamily: "var(--font-head)", fontSize: "0.82rem", fontWeight: 700, color: "var(--muted)", textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: "0.9rem" },
  heroCard: { background: "linear-gradient(145deg, rgba(45,106,79,.35) 0%, rgba(45,106,79,.1) 100%)", border: "1px solid rgba(45,106,79,.4)", borderRadius: 12, padding: "1.25rem", display: "flex", flexDirection: "column" as const, gap: "0" },
  heroBadge: { display: "inline-flex", alignItems: "center", gap: "0.35rem", background: "rgba(232,160,69,.2)", border: "1px solid rgba(232,160,69,.4)", color: "var(--accent)", fontSize: "0.7rem", fontWeight: 700, padding: "0.2rem 0.65rem", borderRadius: 99, marginBottom: "0.6rem", letterSpacing: "0.05em", textTransform: "uppercase" as const, alignSelf: "flex-start" },
  heroName: { fontFamily: "var(--font-head)", fontSize: "1.1rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.3rem", lineHeight: 1.25 },
  heroCode: { background: "rgba(0,0,0,.25)", padding: "0.1rem 0.4rem", borderRadius: 4, fontFamily: "monospace", fontSize: "0.75rem" },
